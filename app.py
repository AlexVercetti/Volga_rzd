...
def start_page():
    ...
    # authentication
    if side_menu_idx == 0:
        st.subheader("Авторизуйтесь пожалуйста")
        ...
        if st.button("Войти"):
            ...
            if result:
                ...
                st.warning("Incorrect User")
    elif side_menu_idx == 1:
        st.subheader("Создание нового аккаунта")
        first_name = st.text_input("Фамилия")
        name = st.text_input("Имя")
        email = st.text_input("Email")
        new_password = st.text_input("Password", type='password')
        if st.button("Зарегистрироваться"):
            add_userdata(first_name, name, email, new_password)
            st.success("Вы успешно создали новый аккаунт")
